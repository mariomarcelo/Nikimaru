  ...
  return (
    <div className="flex flex-col h-[350px] bg-black/60 border border-gold/20 rounded-lg font-mono">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3 text-[11px] text-white">
        {messages.length === 0 && <div className="text-gray-500 italic">[SISTEMA LISTO PARA MARIUS]</div>}
        {messages.map((m) => (
      ...
      <form onSubmit={handleSubmit} className="p-2 border-t border-gold/20 flex gap-2">
        <input value={input} onChange={handleInputChange} placeholder="Escribe comando..." className="flex-1 bg-transparent text-white text-[11px] outline-none" />
        <button type="submit" disabled={isLoading} className="text-gold"><Send className="w-4 h-4" /></button>
      </form>
    </div>
  );
}