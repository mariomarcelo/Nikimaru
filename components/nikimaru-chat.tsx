...
const generateAIResponse = (
  ...
  if (questionLower.includes('price') || questionLower.includes('precio')) {
    return `[NIKIMARU_AI] Current BTC price: $${price.toLocaleString()}. ${
      isHuellaActive 
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed right-0 top-1/2 -translate-y-1/2 z-50 p-2 bg-card border border-border rounded-l-lg transition-all lg:hidden ${
      ...
      {/* Chat Panel */}
      <div
        className={`fixed lg:relative right-0 top-0 h-full w-80 bg-card border-l border-border z-40 transition-transform duration-300 flex flex-col ${
