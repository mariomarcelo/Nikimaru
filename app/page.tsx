...
import { Zap, Trash2, X, Send, Bot, Maximize2, Minimize2, Target, ShieldAlert, TrendingUp, Gauge } from 'lucide-react';

export default function NikimaruLeverageTerminal() {
  ...
  // ESTADO DE APALANCAMIENTO (Leverage)
  ...
  // FUNCIÓN DE TRADING (Apalancamiento inyectado aquí)
  const handleTrade = (type: 'LONG' | 'SHORT') => {
    ...
    const newPos = {
      ...
      type,
      entry: entry,
      leverage: leverage, // <--- Ahora se guarda el valor del slider
      amount: 1000,
      tp: tpPrice,
      sl: slPrice
    };
      ...
      <div className="flex flex-1 overflow-hidden relative">
                ...
                <div key={p.id} className="flex items-center justify-between bg-black/30 p-2 rounded-xl mb-1 border-l-2 border-[#f0b90b]">
                  <div className="flex flex-col">
                    <span className={`text-[10px] font-black ${p.type === 'LONG' ? 'text-[#02c076]' : 'text-[#f84960]'}`}>
                      {p.type} {p.leverage}x | Size: ${p.amount.toFixed(0)}
                    </span>
                    <span className="text-[9px] text-zinc-500">In: {p.entry} | TP: {p.tp || '-'}</span>
                  </div>
        ...
        {!isFullScreen && (
          <div className="w-[280px] border-l border-[#2b2f36] bg-[#161a1e] p-4 flex flex-col gap-3 z-40 overflow-y-auto">
            ...
            {/* SELECTOR DE APALANCAMIENTO FUNCIONAL */}
            <div className="bg-black/20 p-3 rounded-2xl border border-zinc-800">
              ...
              <input
                ...
                className="w-full h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer accent-[#f0b90b]"
              />
              <div className="flex justify-between text-[8px] text-zinc-600 mt-2 font-black">
