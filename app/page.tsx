...
export default function NikimaruLivePNLTerminal() {
  ...
  // MOTOR DE PRECIO (Simulación)
  useEffect(() => {
    const interval = setInterval(() => {
      const p = livePrice + (Math.random() - 0.5) * 15; // Mayor volatilidad para ver el PNL
      setLivePrice(p);
  ...
  // FUNCIÓN PARA CALCULAR PNL Y % EN TIEMPO REAL
  const getPNL = (p: any) => {
    ...
    <div className="min-h-screen bg-[#0b0e11] text-[#eaecef] font-sans flex flex-col overflow-hidden relative select-none">
      {!isFullScreen && (
        <div className="h-12 border-b border-[#2b2f36] flex items-center px-4 bg-[#161a1e] justify-between z-50">
          ...
          <button onClick={() => setIsFullScreen(true)} className="bg-[#2b3139] px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-[#f0b90b] hover:text-black">MODO ANÁLISIS</button>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden relative">
                ...
                return (
                  <div key={p.id} className={`flex items-center justify-between bg-black/40 p-3 rounded-xl mb-2 border-l-4 ${stats.isProfit ? 'border-[#02c076]' : 'border-[#f84960]'}`}>
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] text-zinc-500">In: {p.entry.toFixed(1)}</span>
                    </div>
                    <div className="text-right flex flex-col items-end mr-4">
                      <span className={`text-[12px] font-black ${stats.isProfit ? 'text-[#02c076]' : 'text-[#f84960]'}`}>
                        {stats.isProfit ? '+' : ''}{stats.pnl} USDT
                      </span>
                      <span className={`text-[10px] font-bold ${stats.isProfit ? 'text-[#02c076]' : 'text-[#f84960]'}`}>
                        {stats.isProfit ? '+' : ''}{stats.roe}%
                      </span>
                    </div>
                    <div className="flex gap-1">
                      {[25, 50, 100].map(pct => (
                        <button key={pct} onClick={() => closePartial(p.id, pct)} className="bg-[#2b3139] hover:bg-zinc-700 text-[9px] px-2 py-1.5 rounded font-black">
                          {pct === 100 ? 'CLOSE' : `${pct}%`}
                        </button>
                      ))}
                    </div>
        ...
        {!isFullScreen && (
          <div className="w-[280px] border-l border-[#2b2f36] bg-[#161a1e] p-4 flex flex-col gap-3 z-40 overflow-y-auto">
            ...
            <button onClick={() => handleTrade('LONG')} className="bg-[#02c076] py-3.5 rounded-xl font-black uppercase text-xs shadow-lg active:scale-95 transition-all">Buy / Long</button>
            <button onClick={() => handleTrade('SHORT')} className="bg-[#f84960] py-3.5 rounded-xl font-black uppercase text-xs shadow-lg active:scale-95 transition-all">Sell / Short</button>
          </div>
        )}
        ...
        <div style={{ bottom: `${botPos.y}px`, right: `${botPos.x}px` }} className="fixed z-[200] flex flex-col items-end pointer-events-none">
          {isChatOpen && (
            <div className="w-64 h-72 bg-[#161a1e] border border-[#f0b90b]/30 rounded-3xl shadow-2xl flex flex-col overflow-hidden mb-4 pointer-events-auto">
              ...
              <div className="flex-1 p-4 text-[10px] text-[#f0b90b]">¡Ganancias activadas! Ahora podés ver tu ROE% en tiempo real. ¡A por esos verdes!</div>
            </div>
          )}
          <div onMouseDown={onMouseDown} className="pointer-events-auto p-4 bg-[#f0b90b] rounded-full shadow-2xl cursor-grab active:cursor-grabbing">
